...
export default class MarkUpCode {
  ...
  static getMenuHtml(data) {
    ...
    for (let [name, items] of data) {
      ...
      retVal += `<li class="ecl-menu__item ${bla1}" ${bla2} 
        ...
        <a class="ecl-menu__link" data-ecl-menu-link style="cursor: pointer;" tabindex="0">${name}</a>`
  ...
  static getSubMenuItems(parentMenuItem, items) {
    ...
    items.forEach((name) => {
      retVal += `<li class="ecl-menu__subitem" data-ecl-menu-close data-ecl-menu-subitem mid="${name}" pmid="${parentMenuItem}"><a class="ecl-menu__sublink" style="cursor: pointer;" tabindex="0">${name}</a></li>`
    })
